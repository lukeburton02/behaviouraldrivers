# Plot YouGov drivers per week stratified by region
...
# Load data -----
yougov_weekly_region <- read_csv("data-processed/yougov_weekly_region.csv", show_col_types = FALSE) |>
...
yougov_long <- yougov_weekly_region |>
  select(week_start, region, all_of(names(driver_labels))) |>
  pivot_longer(
    cols      = all_of(names(driver_labels)),
    names_to  = "driver",
    values_to = "value"
  ) |>
  mutate(driver = factor(driver, levels = names(driver_labels), labels = driver_labels))
...
# Plot -----
p <- ggplot(yougov_long, aes(x = week_start, y = value, colour = region)) +
  geom_rect(
    data = lockdowns,
  ...
  labs(
    title    = "YouGov COVID-19 Behavioural Tracker - UK weekly drivers by region",
...
# Save plot -------
ggsave("outputs/yougov_region_drivers.png", p,
       width = 12, height = 10, dpi = 300, bg = "white")
message("Saved: outputs/yougov_region_drivers.png")
