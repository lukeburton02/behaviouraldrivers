# Plot YouGov drivers per week stratified by gender
...
# Load data -----
yougov_weekly_gender <- read_csv("data-processed/yougov_weekly_gender.csv", show_col_types = FALSE) |>
...
yougov_long <- yougov_weekly_gender |>
  select(week_start, gender, all_of(names(driver_labels))) |>
  pivot_longer(
...
# Plot -----
p <- ggplot(yougov_long, aes(x = week_start, y = value, colour = gender)) +
  geom_rect(
    data = lockdowns,
...
# Save plot -------
ggsave("outputs/yougov_gender_drivers.png", p,
       width = 12, height = 10, dpi = 300, bg = "white")
message("Saved: outputs/yougov_gender_drivers.png")
