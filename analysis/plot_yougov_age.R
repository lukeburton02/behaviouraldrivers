# Plot YouGov drivers per week stratified by age group
...
# Load data -----
yougov_weekly_age <- read_csv("data-processed/yougov_weekly_age.csv", show_col_types = FALSE) |>
...
yougov_long <- yougov_weekly_age |>
  select(week_start, age_group, all_of(names(driver_labels))) |>
  pivot_longer(
...
# Plot -----
p <- ggplot(yougov_long, aes(x = week_start, y = value, colour = age_group)) +
  geom_rect(
    data = lockdowns,
...
# Save plot -------
ggsave("outputs/yougov_age_drivers.png", p,
       width = 12, height = 10, dpi = 300, bg = "white")
message("Saved: outputs/yougov_age_drivers.png")
